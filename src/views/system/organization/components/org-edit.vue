...
<template>
	<ele-modal width="680px" :visible="visible" :close-on-click-modal="true" custom-class="ele-dialog-form"
		:title="isUpdate ? '修改机构' : '添加机构'" @update:visible="updateVisible">
		<el-form ref="form" :model="form" :rules="rules" label-width="82px">
			<el-row :gutter="15">
				<el-col v-bind="styleResponsive ? { sm: 12 } : { span: 12 }">
					<el-form-item label="上级机构:">
						<org-select ref="orgRef" v-model="form.parentId" placeholder="不选, 则是根节点" :isSuperAdmin="isSuperAdmin" :tenantId="user.tenantId" />
					</el-form-item>
					...
					<el-form-item label="机构全称:" prop="organizationFullName">
						<el-input clearable :maxlength="100" v-model="form.organizationFullName"
							placeholder="请输入机构全称" />
					</el-form-item>
				...
				<el-col v-bind="styleResponsive ? { sm: 12 } : { span: 12 }">
					<el-form-item label="机构类型:" prop="organizationType">
						<org-type-select ref="orgTypeRef" v-model="form.organizationType" />
					</el-form-item>
					<el-form-item label="排序号:" prop="sortNumber">
						<el-input-number :min="0" :max="99999" v-model="form.sortNumber" controls-position="right"
							placeholder="请输入排序号" class="ele-fluid ele-text-left" />
					</el-form-item>
					<el-form-item label="备注:">
						<el-input :rows="4" type="textarea" :maxlength="200" v-model="form.comments"
							placeholder="请输入备注" />
					</el-form-item>
				</el-col>
			</el-row>
			...
			visible(visible) {
				if (visible) {
					if (this.data) {
						...
						this.form.parentId = this.parentId=='-1'?'':this.parentId;
						this.isUpdate = false;
					}
					this.$nextTick(()=>{
						this.$refs.orgTypeRef.getData();
