<!-- 编辑弹窗 -->
<template>
	<ele-modal width="720px" :visible="visible" :close-on-click-modal="true" custom-class="ele-dialog-form"
		:title="isUpdate ? '修改菜单' : '添加菜单'" @update:visible="updateVisible">
		<el-form ref="form" :model="form" :rules="rules" label-width="92px">
			<el-row :gutter="15">
				<el-col v-bind="styleResponsive ? { sm: 12 } : { span: 12 }">
					<el-form-item label="上级菜单:">
						<ele-tree-select clearable v-model="form.parentId" :data="menuListComp" label-key="title"
							value-key="menuId" default-expand-all placeholder="请选择上级菜单" />
					</el-form-item>
				...
				<el-col v-bind="styleResponsive ? { sm: 12 } : { span: 12 }">
					...
					<el-form-item label="打开方式:">
						<el-radio-group v-model="form.openType" :disabled="form.menuType === 1"
			...
			<el-row :gutter="15">
				<el-col v-bind="styleResponsive ? { sm: 12 } : { span: 12 }">
					...
					<el-form-item name="path">
						<template v-slot:label>
							<el-tooltip v-if="form.openType === 2" placement="top"
						...
						<el-input clearable v-model="form.path" :disabled="form.menuType === 1"
							:placeholder="pathPlaceholder" />
					</el-form-item>
					<el-form-item name="component">
						<template v-slot:label>
							<el-tooltip v-if="form.openType === 1" placement="top"
								content="需要以`http://`、`https://`、`//`开头">
								<i class="el-icon-_question"></i>
							</el-tooltip>
							<span>
						...
						<el-input clearable v-model="form.component" :disabled="
							form.menuType === 1 ||
							form.openType === 2
				...
				<el-col v-bind="styleResponsive ? { sm: 12 } : { span: 12 }">
					...
					<el-form-item label="排序号:" prop="sortNumber">
						<el-input-number :min="0" v-model="form.sortNumber" placeholder="请输入排序号"
							controls-position="right" class="ele-fluid ele-text-left" />
					</el-form-item>
					<el-form-item label="是否展示:">
						<el-switch :active-value="0" :inactive-value="1" v-model="form.hide"
			...
			<el-form-item label="路由元数据:" prop="meta">
				<el-input :rows="4" type="textarea" :maxlength="200" v-model="form.meta"
...
<script>
	...
	export default {
				...
				rules: {
					...
					meta: [{
						...
						validator: (_rule, value, callback) => {
							if (value) {
								const error = new Error('请输入正确的JSON格式数据');
								try {
		...
		computed: {
			...
			menuListComp() {
				var res = this.menuList.filter(item => {
