<template>
	<el-card shadow="never" body-style="padding: 0;">
		<div class="ele-cell demo-monitor-tool">
			...
			<div :class="['ele-inline-block', { 'hidden-sm-and-down': styleResponsive }]"
				style="width: 260px; margin-left: 10px">
				<el-date-picker unlink-panels type="daterange" class="ele-fluid" end-placeholder="结束日期"
					start-placeholder="开始日期" v-model="saleSearch.datetime" range-separator="至" size="small" />
			</div>
		</div>
		<el-divider />
		<el-row>
			<el-col v-bind="styleResponsive ? { lg: 18, md: 16 } : { span: 18 }">
				...
				<v-chart ref="saleChart" style="height: 285px" :option="saleChartOption" />
			</el-col>
			<el-col v-bind="styleResponsive ? { lg: 6, md: 8 } : { span: 6 }">
				...
				<div v-for="(item, index) in saleroomRankData" :key="index" class="demo-monitor-rank-item ele-cell"
					...
					<el-tag size="mini" type="info" :effect="index < 3 ? 'dark' : 'light'"
						:color="index < 3 ? '#314659' : 'hsla(0, 0%, 60%, .2)'" style="border-color: transparent"
						class="ele-tag-round">
						{{ index + 1 }}
					</el-tag>
