<template>
	<div class="ele-body ele-body-card">
		...
		<vue-draggable tag="el-row" v-model="data" handle=".el-card__header" :component-data="{ props: { gutter: 15 } }"
			:animation="300" :set-data="() => void 0" @end="onEnd">
			<el-col v-for="(d, i) in data" :key="d.name"
				v-bind="styleResponsive ? { md: d.md, sm: d.sm } : { span: d.md }">
				<component :is="d.name" :title="d.title" @remove="onRemove(i)" @edit="onEdit(i)" />
			</el-col>
		</vue-draggable>
		...
		<ele-modal width="680px" :visible.sync="visible" title="未添加的视图">
			<el-row :gutter="15" style="margin-bottom: -15px">
				<el-col v-for="item in notAddedData" :key="item.name" :md="8" :sm="24">
					<div class="workplace-card-item ele-border-lighter" @click="addView(item)">
